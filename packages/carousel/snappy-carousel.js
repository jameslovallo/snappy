		...
		case false:
			class carousel extends HTMLElement {
				constructor() {
					...
					this.shadowRoot.innerHTML = /* html */ `
						<style>
							...
							.c > button svg,
							.c__indicators svg,
							[name='prev-icon']::slotted(svg),
							[name='next-icon']::slotted(svg) {
								display: block;
								height: var(--arrow-size, 48px);
								width: var(--arrow-size, 48px);
				...
				connectedCallback() {
					...
					const iActiveSlot = c.querySelector('.c__i__active').assignedElements()[0]
					...
					this.toggleControls = () => {
						const display = track.scrollWidth === track.offsetWidth ? 'none' : ''
					...
					const announce = () => {
						const current = this.slides.filter((slide) => slide.ariaHidden === 'false')
